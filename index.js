...
io.on("connection", (socket) => {
  ...
  socket.on("sendMessage", (message) => {
    console.log(
      onlineUsers.find((user1) => user1.userId == message.recipientId)
    );
    const user = onlineUsers.find(
      (user1) => user1.userId === message.recipientId
    );
  ...
  socket.on("sendMarkRead", (req) => {
    const user = onlineUsers.find((user1) => user1.userId === req.recipientId);
    if (!user) return;
    io.to(user.socketId).emit("getMarkRead", req.chatId);
  });
  ...
  socket.on("callUser", (data) => {
    ...
    if (calluser) console.log("send call");
  ...
  socket.on("answerCall", (data) => {
    ...
    if (calluser) io.to(calluser.socketId).emit("callAccepted", data.signal);
  });

  socket.on("callend", (data) => {
    const calluser = onlineUsers?.find((users) => users.userId === data);
    if (calluser) io.to(calluser.socketId).emit("callEnded");
  });
